...
final class TikTokService {
    ...
    private let tiktokAppID = "7595356221092233223"
    private let accessToken = "TTld9wfYR8FAoRffIxkboIaEepSGmDMo"
    ...
    func trackHomeViewed() {
        trackTTEvent("HomeViewed")
        print("📊 TikTok: Home View tracked")
    }
    ...
    // MARK: - A/B/C Experiment Events
    
    /// Track experiment paywall viewed with variant attribution
    func trackExperimentPaywallViewed(variant: String, source: String) {
        trackTTEvent("ExperimentPaywallViewed", properties: [
            "funnel_variant": variant,
            "source": source
        ])
        print("📊 TikTok: Experiment Paywall Viewed - Variant \(variant)")
    }
    
    /// Track experiment checkout initiated
    func trackExperimentCheckout(variant: String, planType: String, price: Double) {
        trackTTEvent("ExperimentCheckout", properties: [
            "funnel_variant": variant,
            "plan_type": planType,
            "value": price,
            "currency": "USD"
        ])
        print("📊 TikTok: Experiment Checkout - Variant \(variant) - \(planType)")
    }
    
    /// Track experiment purchase completed
    func trackExperimentPurchase(variant: String, planType: String, price: Double) {
        trackTTEvent("ExperimentPurchase", properties: [
            "funnel_variant": variant,
            "plan_type": planType,
            "value": price,
            "currency": "USD"
        ])
        print("📊 TikTok: Experiment Purchase - Variant \(variant) - $\(price)")
    }
    
    /// Track micro-engagement events (for Variant C commitment flow)
    func trackExperimentEngagement(variant: String, action: String, value: String) {
        trackTTEvent("ExperimentEngagement", properties: [
            "funnel_variant": variant,
            "action": action,
            "value": value
        ])
        print("📊 TikTok: Experiment Engagement - \(action): \(value)")
    }
}
