...
extension Image {
    ...
    @MainActor
    static var horizontalScrollFadeMask: Image {
        if let cached = _horizontalScrollFadeMaskCache {
            return cached
        }
        ...
        let image = ImageRenderer(
            content: HStack(spacing: 0) {
                // Left: Opaque (Blur) -> Transparent (No Blur)
                LinearGradient(colors: [.black, .clear], startPoint: .leading, endPoint: .trailing)
                    .frame(width: 40)
                
                // Center: Transparent (No Blur)
                Color.clear
                
                // Right: Transparent (No Blur) -> Opaque (Blur)
                LinearGradient(colors: [.clear, .black], startPoint: .leading, endPoint: .trailing)
                    .frame(width: 60)
            }
            .frame(width: width, height: 200) // Hauteur suffisante
        )
        
        let result = Image(uiImage: image.uiImage ?? UIImage())
        _horizontalScrollFadeMaskCache = result
        return result
    }
    ...
    @MainActor private static var _horizontalScrollFadeMaskCache: Image?
}
