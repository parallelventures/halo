...
struct EclatSecondaryButton: View {
    let title: String
    let icon: String?
    let action: () -> Void
...
#Preview("Buttons") {
    VStack(spacing: 20) {
        EclatCTAButton("Continue", icon: "arrow.right") {}
        EclatCTAButton("Loading...", isLoading: true) {}
        EclatSecondaryButton("Skip", icon: "xmark") {}
        
        HStack {
            EclatIconButton(icon: "xmark") {}
            EclatIconButton(icon: "camera") {}
            EclatIconButton(icon: "photo") {}
        }
        
        HStack {
            EclatChip("Popular", icon: "flame.fill", isSelected: true)
            EclatChip("New", icon: "sparkles")
            EclatChip("Trending")
        }
    }
    .padding()
